	...
	public static void main(String[] args) {
		 ...
		 */
			...
			/*
			 	...
			 	새로 추가할 값이 같은지 검사한다.
			 	3. 두 값이 같다면 데이터를 중복으로 저장하지 않고, 두 값이 다르다면 저장된 값의
			 	다음 위치에 새로운 값을 저장한다.
			 */
			System.out.println("===============================");
			...
			hs2.add(new Student("공유", 40, 100));
			hs2.add(new Student("홍길동", 40, 60));
			hs2.add(new Student("류수정", 70, 10));
			hs2.add(new Student("명지원", 90, 20));
			hs2.add(new Student("김갑생", 25, 46));
			
			System.out.println(hs2);
			System.out.println(hs2.size());
			
			Student s1 = new Student("명지원", 30, 40);
			Student s2 = new Student("나다", 50, 60);
			...
			//2. Iterator를 통한 반복
			// - List계열과 Set계열에서는 Iterator를 직접 구현하였기 때문에 사용이 가능하나,
			...
			// List <--> Set
			//1. List에 Set 계열 데이터를 추가하는 방법
			List<Student> list = new ArrayList<>(hs2);
			
			//2. List에 Set계열 데이터를 추가하는 방법
			// addAll()
			List<Student> list2 = new ArrayList<>(hs2);
			list2.addAll(list2);
