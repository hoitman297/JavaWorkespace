...
public class Run {
	public static void main(String[] args) {
		/*
		 	...
		 	싱글 스레드
		 	- 메인 스레드 1개 가지고 작업을 처리 한다.
		 	- 한 작업씩 순차적으로 처리한다.
		 	
		 	멀티 스레드
		 	- 메인 스레드와 추가 스레드를 이용하여 동시에 여러 작업을 처리 한다.
		 	
		 	멀티 스레드의 장점
		 	1) 작업의 효율성 증대
		 		- A 쓰레드 B 쓰레드 나눠서 작업을 처리하므로 훨씬 더 빨리 작업완료가 가능.
		 	2) 응답성 향상
...
//			th1.start();
			...
			/*
			 	...
			 	실제로는 OS의 "스케쥴러"에 의해 스레드의 전호나이 순식간에 이루어져 동시에 실행되는 것처럼 보이는 현상이다.
