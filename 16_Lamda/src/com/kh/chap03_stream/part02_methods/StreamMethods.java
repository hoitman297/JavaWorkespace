...
public class StreamMethods {
	public static void main(String[] args) {
		/*
		 	...
		 	3) Calaulating 계열 : min, max, sum, average 등
		 		-> 산술연산관련되 최정 처리 메서드들 (기본자료형스트림에만 존재)
		 	4) Matching : anyMatch, allMatch, noneMatch...
		 		-> 스트림 내부 요소에서 특정 조건을 만족하는 요소를 선택하기 위해 사용
		 	5) Ruduction 계열 : reduce 메서드를 통해 요소 내부의 값을 다양한 방법으로 연산 후 결과 값을 반환핳는 메서드.
		 								   Calculating 계열에서 제공하지 않는 연산기능을 수행할 때 사용한다.
		 	
		 */
		// 중간처리 메서드(distinct, filter, forEach)
		List<Integer> list = Arrays.asList(5,1,2,3,4,2,3,2,3,4,5,1);
		...
		String name[]	 = {"강감찬","강원래","강형욱","홍길동","초난강"};
		
		Arrays
			.stream(name)
			.filter(names -> names.startsWith("강"))
		...
		//4) mapTo(Int / Double / Long) : Stream 을 기본자료형스트림으로 변경해주는 메서드.
			// 4) Collect 계열 메서드
			// 4-1) Collectors.toList() : 스트림의 List를 메서드로 변환
			List<Integer> newList1 = list2.stream().map(i -> i * 100).collect(Collectors.toList());
			...
			//	4-3) Collectors.toMap (Function, Function) : 스트림의 결과를 Map으로
				...
				//5-3) summaryStatisstics : 스트림 요소의 통계값을 가진 객체를 반환해주는 메서드.
				//	통계값 -> 총 개수, 합, 평균, 최소, 최대
				...
				IntSummaryStatistics summ =
						IntStream.of(arr.get(0).getAge(),arr.get(1).getAge(),arr.get(2).getAge(),arr.get(3).getAge(),arr.get(4).getAge())
						.summaryStatistics();
