...
public class OptionalMethods {
	/*
	 	Optional
	 		...
	 		- Optional 객체를 변수에 저장하여 관리하는 경우, 변수명에 maybe, opt와 같은 Optional 타입을 암묵적으로 제시한다.
	 */
	public static void main(String[] args) {
		/*
		 	Optional 객체 생성 방법
		 	1. Optional.empty() : 값이 비어있는 Optional 객체 생성
		 	2. Optional.of(obj) : 결코 null이 아닌 객체를 담은 Optional객체 생성
		 	3. Optional.ofNullable(obj) : null 일수도 있는 Optional 객체 생성
		 	
		 	Optional 객체 내부 메서드들
		 		- get() : Optional 내부의 값을 반환하는 메서드.(값이 없다면 NoSuchElement 예외발생
				- getAs자료형() : 해당 자료형의 값을 반환하는 메서드
				- orElse(T) : get()으로 꺼낼 값이 존재하지 않을 경우 반환할 값.
				- orElseGet(Supplier) : get()으로 꺼낼값이 존재하지 않을 경우 람다식을 통해 구현한 값을 반환
				- orElseThrow(Supplier) : get()으로 꺼낼값이 없을때, 람다식을 통해 구현한 에러객체를 호출
		 */
...
//		//Optional 없이 최대한 안전(NPE)하게 email 추출하기
//		if(order != null) {
