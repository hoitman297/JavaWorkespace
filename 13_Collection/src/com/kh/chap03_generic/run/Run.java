...
//		Generic.<String>printTest(111111);
		...
		//3. 제네릭의 extends 문법
		/*
		 - 제네릭에서의 extends는 클래스간의 상속 개념이 아닌, 
		 	제네릭으로 지정한 자료형의 범위를 "제한" 시키는데 사용한다.
		 - <T extends Paraent> : T (타입변수) 에는 Parent클래스나 Parent 클래스를
		 	상속받은 자료형만 대입 가능
		 - <T extends Interface> : Interface를 구현한 클래스만 대입 가능.
		 */
		GenericExtends<Parent> parent = new GenericExtends<>();
		GenericExtends<Child1> chiled1 = new GenericExtends<>();
		// Parent 및 Parent자식이 아니기 때문에 타입 설정 오류
//		GenericExtends<String> str = new GenericExtends<>();
		...
		/*
		 	...
		 	- 와일드카드(?)의 의미는 어떤 자료형이 들어올지 모른다(unknown)의 의미로 사용하며,
		 		모든 자료형(any)로 들어올 수 있다는 의미는 아니다.
		 	- ?는 extends와 super 예약어로 자료형의 범위를 제한할 수 있다.
		 	- ? extends T : 상한제한 => 와일드카드의 범위를 T 또는 T의 자식들을 제한하는 기능.
...
//		unknown2.setType(new Child1()); // ?의 범위에 Child2도 포함되어 있으므로 대입 불가능
		//한상황이 생긴다.
//		unknown2.setType(new Child2());
		
		//unknown2가 child1이든 chold2든 parent든, 다향성에 의해 값을 추출차형 대입 가능.
		// 즉, extends를 통한 상한 제한은 데이터를 "가져왔을댸" 자주 사용
		...
		unknown3.setType(new Child1());  //다향성의 업캐스팅으로 문제 없이 저장 가능.
		unknown3.setType(new Child2());
		
		//값을 "대입"하고자 할때는 super를 통한 하한제한을 사용하는 것이 적절하다.
		
		Parent p3 = unknown3.getType(); // ?는 Object일 수 있음. 단, 클래스 레벨 제네릭 설정시
		//T extends Parent로 설정했기 때문에 T의 범위는 Parent ~ Parent의 자식클래스로 한정.
		
		//따라서 타입 변수에 extends를 통한 타입제한이 없는 경우, 데이터는 항상 Object형태로만 추출이 가능.
