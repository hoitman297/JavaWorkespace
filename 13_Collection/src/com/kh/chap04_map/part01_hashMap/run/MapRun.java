...
public class MapRun {
	public static void main(String[] args) {
		/*
		 * HashMap - 많은 양의 데이터를 "빠르게 저장하고 관리"하기 위하여 사용하는 클래스로, key값과 value값을 함께 저장한다. -
		 * 계층구조상 List, Set계열과 다른 Map계열 인터페이스를 상속한 클래스로, 사용하는 메서드가 다르다. - 데이터를 저장할 때 key,
		 * value를 한 쌍으로 저장하고, key를 통해 value가 저장된 위치에 접근한다. - key값의 중복을 허용하지 않는다. - 동일한
		 * key값에 데이터를 저장하면, 마지막으로 추가된 value 값이 저장된다.
		 */
		...
		hm.put("두쫀쿠", new Snack("초코맛", 6000));
		hm.put("포테이토 칩", new Snack("짠맛", 500));
		...
		// - hashmap에서 key값과 일치하는 값을 찾아, 새로 전달된 value롤
		// "교체"하는 메서드
