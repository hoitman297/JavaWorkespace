...
public class ListRun {
	/*
	 	...
	 	1. 같은자료형의 데이터만 저장가능
	 	2. 배열의 크기를 수정할 수 없다.
	 	3. 배열내부의 값들을 조직하기 불편하다.
	 	
	 	- 컬렉션의 장점
	 	1. 여러자료형의 값을 저장할 수 있다(제네릭 설정을 통해 한가지 유형의 데이터만 저장도 가능)
	 	2. 중간에 값을 추가하거나, 삭제하는 등 배열내부의 값들을 조작하기위한 편리한 메서드가 정의 되어 있다.
	 	...
	 	- 중복값을 담아도 상관없다.
	 	- ArrayList(많이 사용됨), Vector, LinkedList 등이 존재
		...
		Map 계열
		- 담고자 하는 값(value)와 그 값을 저장하는 키(key)를 함께 저장
	...
	public static void main(String[] args) {
		...
		/*
			제네릭 ( <E> )
			 - 별도의 제네릭설정없이 컬렉션 객체 생성시 제네릭은 Object로 설정된다.
		...
		ArrayList<Music> list1 = new ArrayList<>(3); //객체생성시에는 제네릭 생략가능.
		
		System.out.println(list); //size 기준 내용 출력
		//1. add(E e) : 리스트의 끝(초기화된 위치 기준)에 전달된 e를 추가시켜주는 메서드
		
		list1.add(new Music("Tears" , "소찬휘"));
		list1.add(new Music("와" , "이정현"));
		list1.add(new Music("ajd" , "김현정"));
		...
		// - 리스트에 초기화된 데이터 수를 반환하는 메서드
		 ...
		 List<Music> list2 = list1.subList(0,  2);
